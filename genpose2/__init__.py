...
try:
    from .networks.pts_encoder.pointnets import PointNetfeat as _OriginalPointNetfeat
except Exception:  # pragma: no cover - kept as a runtime fallback for missing optional deps.
    _OriginalPointNetfeat = None


class _FallbackPointNetfeat(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        return self.net(x).amax(dim=2)


class R2GenPose2PoseHead(nn.Module):
    """R2 adapter around GenPose2's bundled PointNet point-cloud encoder."""

    def __init__(self, num_points: int = 1024, feature_dim: int = 512, hidden_dim: int = 256):
        super().__init__()
        self.num_points = int(num_points)
        if _OriginalPointNetfeat is not None:
            self.encoder = _OriginalPointNetfeat(num_points=self.num_points, in_dim=7, out_dim=int(feature_dim), global_feat=True)
        else:
            self.encoder = _FallbackPointNetfeat(in_dim=7, out_dim=int(feature_dim))
        self.head = nn.Sequential(
            nn.Linear(int(feature_dim), int(hidden_dim)),
            nn.ReLU(inplace=True),
            nn.Linear(int(hidden_dim), int(hidden_dim)),
            nn.ReLU(inplace=True),
            nn.Linear(int(hidden_dim), 9),
        )
...
def _make_points(hand_som: torch.Tensor, object_data: torch.Tensor, num_points: int) -> torch.Tensor:
    ...
    for hand, obj in zip(hand_som, object_data):
        ...
        obj_feat = torch.cat([obj_xyz, torch.zeros((n_obj, 1), device=obj_xyz.device), torch.ones((n_obj, 1), device=obj_xyz.device), torch.zeros((n_obj, 2), device=obj_xyz.device)], dim=1)
        hand_feat = torch.cat([hand_xyz, pressure, torch.zeros((n_hand, 1), device=hand_xyz.device), torch.ones((n_hand, 1), device=hand_xyz.device), torch.zeros((n_hand, 1), device=hand_xyz.device)], dim=1)
...
def _evaluate(model: nn.Module, dataset: Any, indices: np.ndarray, args: Any, device: torch.device) -> Dict[str, Any]:
    ...
    num_points = int(getattr(args, "genpose2_num_points", 1024))
    batch_size = int(getattr(args, "genpose2_eval_batch_size", args.batch_size))
...
def train(args, dataset, train_idx, val_idx, test_idx, device: torch.device) -> Path:
    ...
    num_points = int(getattr(args, "genpose2_num_points", 1024))
    model = R2GenPose2PoseHead(
        num_points=num_points,
        feature_dim=int(getattr(args, "genpose2_feature_dim", 512)),
        hidden_dim=int(getattr(args, "genpose2_hidden_dim", 256)),
    ).to(device)
...
def test(args, dataset, test_idx, device: torch.device, checkpoint: Path | None = None) -> Dict[str, Any]:
    ...
    num_points = int(payload.get("num_points", getattr(args, "genpose2_num_points", 1024)))
    model = R2GenPose2PoseHead(num_points=num_points, feature_dim=int(getattr(args, "genpose2_feature_dim", 512)), hidden_dim=int(getattr(args, "genpose2_hidden_dim", 256))).to(device)
    model.load_state_dict(payload["state_dict"])
