...
def others_work_dir() -> Path:
    return Path(__file__).resolve().parents[4] / "G_20260304_Rerunall" / "experiment" / "others_work"
...
def handsom_to_contact_mask(hand_som: torch.Tensor, side: int = 28) -> torch.Tensor:
...
def handsom_to_tactile_image(hand_som: torch.Tensor, side: int = 64) -> torch.Tensor:
    return torch.nn.functional.interpolate(handsom_to_contact_mask(hand_som, side=28), size=(side, side), mode="bilinear", align_corners=False)
